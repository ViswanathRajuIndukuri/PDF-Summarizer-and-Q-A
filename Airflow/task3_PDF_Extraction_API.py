...
convert_api_instance = cloudmersive_convert_api_client.ConvertDocumentApi(cloudmersive_convert_api_client.ApiClient(convert_configuration))
...
ocr_api_instance = cloudmersive_ocr_api_client.ImageOcrApi(cloudmersive_ocr_api_client.ApiClient(ocr_configuration))
...
def extract_text_from_images_cloudmersive(pdf_bytes):
    ...
    try:
        ...
        print(f"Exception when calling Cloudmersive OCR API: {e}\n")
        return None
    finally:
        # Clean up the temp file if it exists
        if os.path.exists(temp_pdf_path):
...
def process_pdfs_in_gcp_cloudmersive(bucket_name, source_folder, target_folder):
    """Processes all PDFs in the source folder, extracts content using Cloudmersive APIs (text and images)."""
    bucket = storage_client.bucket(bucket_name)
    ...
    for blob in blobs:
        if blob.name.endswith('.pdf'):
            ...
            extracted_content = (extracted_text_content or "") + "\n" + (extracted_image_content or "")
